...
// CompressionPlugin for Gzip compression
...
// Define CDN resource list for Service Worker access
const cdnResources = {
  css: [
...
module.exports = defineConfig({
  ...
  devServer: {
    ...
    client: {
      overlay: false, // Do not show webpack error overlay
    },
  },
  publicPath: process.env.VUE_APP_PUBLIC_PATH || "/",
  chainWebpack: config => {

    // Modify HTML plugin config to dynamically insert CDN links
    config.plugin('html')
      .tap(args => {
        // Decide whether to use CDN based on config
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      config.plugins.push(
        new InjectManifest({
          ...
          // Add extra info passed to Service Worker
      ...
      if (useCDN) {
        ...
        // Do not set externals when not using CDN so webpack bundles dependencies
        config.externals = {};
      }
      ...
      config.cache = {
        ...
        maxAge: 5184000000, // Cache validity: 1 month
        buildDependencies: {
  ...
  // Expose CDN resource info to service-worker.js
  pwa: {
    workboxOptions: {
      skipWaiting: true,
      clientsClaim: true
