...
export default {
  ...
  mounted() {
    // If this is a mobile device and VUE_APP_H5_URL is set, redirect to the H5 page
    if (this.isMobileDevice() && process.env.VUE_APP_H5_URL) {
      window.location.href = process.env.VUE_APP_H5_URL;
      return;
    }
    
    // Only add the related events and features when CDN is enabled
    if (this.isCDNEnabled) {
      // Add the global Alt+C shortcut to open the cache viewer
      document.addEventListener('keydown', this.handleKeyDown);

      // Expose a cache-check helper on the global object for debugging
      window.checkCDNCacheStatus = () => {
        this.showCacheViewer = true;
      };

      // Log helpful messages to the console
      console.info(
  ...
  methods: {
    ...
    isMobileDevice() {
      // Check whether the user agent belongs to a mobile device
        ...
        try {
          ...
          if (registrations.length > 0) {
            ...
            setTimeout(async () => {
              ...
              // Provide extra hints in development environments
