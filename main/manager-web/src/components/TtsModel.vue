...
export default {
  ...
  methods: {
    ...
    cancelEdit(row) {
      // Remove newly added items from array when cancelling edit
      if (!row.id) {
    ...
    handleResponse(res, row) {
      if (res.code === 0) {
        ...
        // Restore original data when saving fails
        if (row.originalData) {
          Object.assign(row, row.originalData);
          row.editing = false;
          delete row.originalData;
    ...
    addNew() {
      ...
      const newRow = {
        ...
        selected: false,
        editing: true,
        sort: 0 // Newly added data sorted at the top by default
      };
...
/* Button group position adjustment */
.action-buttons {
  padding-top: 10px;
  text-align: left;
}
