        ...
        navigator.serviceWorker
              ...
              installingWorker.onstatechange = () => {
                if (installingWorker.state === 'installed') {
                  if (navigator.serviceWorker.controller) {
                    // Cached content updated, notify user to refresh
                    console.log('[Xiaozhi Service] New content available, please refresh the page');
                    // Show update notification prompt here
                    ...
                    // Everything normal, Service Worker successfully installed
                    ...
                    setTimeout(() => {
                      ...
                      cdnUrls.forEach(url => {
                        fetch(url, { mode: 'no-cors' }).catch(err => {
                          console.log(`Pre-warming cache ${url} failed`, err);
                        });
                      });
                    }, 2000);
                  }
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              console.warn('[Xiaozhi Service] Network error registering Service Worker, CDN resources may not be cached');
              if (process.env.NODE_ENV === 'production') {
                console.info(
                  'Possible causes: 1. Server MIME type incorrect 2. Server SSL certificate issue 3. Server did not return service-worker.js'
                );
              }
            }
          });
