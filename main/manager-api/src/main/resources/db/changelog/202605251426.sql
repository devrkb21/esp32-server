...
`update_date`) VALUES (''SYSTEM_PLUGIN_CALL_DEVICE'', ''Plugin'', ''call_device'', ''Device Call Device'', ''[]'', 85, 1988490863118454785, ''2026-05-18 
12:00:00'', 1988490863118454785, ''2026-05-18 12:00:00'')',
'SELECT ''data already exists, skip'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Update system menu configuration: add contacts management menu
UPDATE sys_params
...
-- Create device contacts table
CREATE TABLE IF NOT EXISTS `ai_device_address_book` (
`mac_address` VARCHAR(64) NOT NULL COMMENT 'Local device MAC address',
`target_mac` VARCHAR(64) NOT NULL COMMENT 'Remote device MAC address',
`alias` VARCHAR(64) DEFAULT NULL COMMENT 'Alias',
`has_permission` TINYINT(1) DEFAULT TRUE COMMENT 'Has permission to call',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Device contacts table';