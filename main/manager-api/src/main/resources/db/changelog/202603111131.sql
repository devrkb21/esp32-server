-- Add parameters returned by ragflow (when creating/querying knowledge base)
...
-- Retain document ID linking remote RAGFlow doc ID with local ID (metadata link only; file contents stored in RAGFlow)
...
CREATE TABLE `ai_rag_knowledge_document` (
     ...
     `parser_config` text COMMENT 'Parsing configuration (JSON)',
     `status` varchar(10) DEFAULT '1' COMMENT 'Available status (1: Enabled, 0: Disabled)',
     `run` varchar(32) DEFAULT 'UNSTART' COMMENT 'Execution status (UNSTART/RUNNING/CANCEL/DONE/FAIL)',
     `progress` double DEFAULT '0' COMMENT 'Parsing progress (0.0 ~ 1.0)',
     `thumbnail` mediumtext COMMENT 'Thumbnail (Base64 or URL)',
     `process_duration` double DEFAULT '0' COMMENT 'Parsing duration (seconds)',
     ...
     `last_sync_at` datetime DEFAULT NULL COMMENT 'Last synchronization time',
     PRIMARY KEY (`id`),
