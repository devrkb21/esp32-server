...
public class ConfigServiceImpl implements ConfigService {
    ...
    public Map<String, Object> getConfig(Boolean isCache) {
        if (isCache) {
            // First fromRedisGet configuration
        ...
        // Construct configurationInformation
    ...
    public Map<String, Object> getAgentModels(String macAddress, Map<String, String> selectedModule) {
        // Check whether request is from management console
        ...
        // According toMACaddress to find device
        DeviceEntity device = deviceService.getDeviceByMacAddress(macAddress);
        if (device == null) {
            // If device，removeredisto check if there are devices that need to be connected
        ...
        if (timbre != null) {
            ...
            // Prioritize user selected language，If none, use the first language supported by the voice timbre
            ...
            if (voice_print != null) {
                ...
                // Prioritize user selected language，If none, then useDefault value
                language = StringUtils.isNotBlank(agent.getTtsLanguage()) ? agent.getTtsLanguage() : "Mandarin";
            }
        }
        // Construct return data
        Map<String, Object> result = new HashMap<>();
        // Get maximum characters output per device per day
        ...
        // IfClientInstantiated model，then do not return
        ...
        // Add function calling parametersInformation
        ...
        // Get context source configuration
     ...
     * Construct configurationInformation
     * 
     * @param config System parametersList
     * @return ConfigurationInformation
     */
    private Object buildConfig(Map<String, Object> config) {

        // QueryAllSystem parameters
        ...
        for (SysParamsDTO param : paramsList) {
            ...
            // Traverse except the last onekeyall exceptkey
            ...
            // Process the last onekey
            String lastKey = keys[keys.length - 1];
            String value = param.getParamValue();

            // According tovalueTypeConvert value
            switch (param.getValueType().toLowerCase()) {
                case "number":
                    try {
                        double doubleValue = Double.parseDouble(value);
                        // If value is in integer form，then convert toInteger
                ...
                case "boolean":
                    ...
                    break;
                case "array":
                    // Convert semicolon-separated string to integer array
    ...
    /**
     * Construct voiceprint configurationInformation
     * 
    ...
    private void buildVoiceprintConfig(String agentId, Map<String, Object> result) {
        try {
            ...
            // Get voiceprint associated with agentInformation（No user permission verification required）
            ...
            // Construct voiceprint configuration
            ...
            // Failure to obtain voiceprint configuration does not affect other functions
            System.err.println("Get voiceprint configurationFail: " + e.getMessage());
        }
    }

    /**
     * Get voiceprint associated with agentInformation
     * 
     * @param agentId AgentID
     * @return VoiceprintInformationList
     */
     ...
     * @param voice          Voice timbre
     * @param referenceAudio Reference audio path
     * @param referenceText  Reference text
     * @param vadModelId     VADModelID
     * @param asrModelId     ASRModelID
     * @param llmModelId     LLMModelID
     * @param ttsModelId     TTSModelID
     * @param memModelId     Memory modelID
     * @param intentModelId  Intent modelID
     * @param result         ResultMap
     */
    private void buildModuleConfig(
        ...
        for (int i = 0; i < modelIds.length; i++) {
            ...
            // Key：Third parameter passedfalse，Ensure raw key is obtained
            ...
            if (model.getConfigJson() != null) {
                ...
                // If it isTTSType，Addprivate_voiceAttribute
                if ("TTS".equals(modelTypes[i])) {
                    ...
                    // Volcengine voice cloning requires replacementresource_id
                    Map<String, Object> map = (Map<String, Object>) model.getConfigJson();
                    if (Constant.VOICE_CLONE_HUOSHAN_DOUBLE_STREAM.equals(map.get("type"))) {
                        // Ifvoiceis”S_”starting with，Useseed-icl-1.0
                        if (voice != null && voice.startsWith("S_")) {
                            map.put("resource_id", "seed-icl-1.0");
                        }
                    }
                }
                // If it isIntentType，andtype=intent_llm，then add additional models to it
                ...
                // If it isLLMType，andintentLLMModelIdNot empty，then add additional models
                if ("LLM".equals(modelTypes[i])) {
                    if (StringUtils.isNotBlank(intentLLMModelId)) {
                        if (!typeConfig.containsKey(intentLLMModelId)) {
                            // UpdateHere：AddisMaskSensitive=falseParameter
                    ...
                    // LLMalso return selectedSLM，If duplicate nameidthen do not display repeatedly
