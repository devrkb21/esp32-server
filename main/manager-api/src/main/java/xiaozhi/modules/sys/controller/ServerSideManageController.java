...
/**
 * ServerManagement controller
 */
@RestController
@RequestMapping("/admin/server")
@Tag(name = "ServerManagement")
@AllArgsConstructor
public class ServerSideManageController {
    ...
    static {
        ...
        // IgnorejsonStringexists in，butpojocase where corresponding field does not exist
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    }

    @Operation(summary = "GetWsServerList")
    ...
    @Operation(summary = "NotificationpythonServerUpdate configuration")
    @PostMapping("/emit-action")
    @LogOperation("NotificationpythonServerUpdate configuration")
    @RequiresPermissions("sys:role:superAdmin")
    ...
    private Boolean emitServerActionByWs(String targetWsUri, ServerActionEnum actionEnum) {
        ...
        redisUtils.set(redisKey, "true", 300); // 5minutes validity period
        ...
        try (WebSocketClientManager client = new WebSocketClientManager.Builder()
                ...
                .headers(headers)
                .build()) {
            // IfConnected successfullythen send ajsonpacket and waitServerResponse
            client.sendJson(
                    ServerActionPayloadDTO.build(
                            actionEnum,
                            Map.of("secret", serverSK)));
            // Wait for server response and continuously listen for messages
            ...
            // Catch all errors，by globalException handlerReturn
            throw new RenException(ErrorCode.WEB_SOCKET_CONNECT_FAILED);
        }
        return true;
    }
