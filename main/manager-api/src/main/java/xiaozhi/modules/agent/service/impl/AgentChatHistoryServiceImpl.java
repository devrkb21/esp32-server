...
public class AgentChatHistoryServiceImpl extends CrudRepository<AiAgentChatHistoryDao, AgentChatHistoryEntity>
    ...
    public List<AgentChatHistoryUserVO> getRecentlyFiftyByAgentId(String agentId) {
        // BuildQueryCondition(Do not add sorting by creation time，Larger primary key naturally corresponds to later creation time
        // Not adding this reduces full scan overhead of sorting all data during pagination)
        ...
        wrapper.select(AgentChatHistoryEntity::getContent, AgentChatHistoryEntity::getAudioId)
                ...
                // AddThis line，Ensure query results are sorted by creation time descending
                // Useidoriginalcause：DataForm，idLargerCreation timethe later it is，allwithUseidresult is identical to sorting by creation time descending
                // iddoasDescendingArrangeoptimizetrend，High performance，hasPrimary keyIndex，No need to perform exclusion scan comparison again during sorting
                .orderByDesc(AgentChatHistoryEntity::getId);

        // BuildPagination query，Querybefore50Pagesaccording
    ...
    /**
     * from content FieldinExtract chatContent
     * If content is JSON Format（e.g. {"speaker": "notNotifyvoiceuser", "content": "nowatWhat time is it。"}），thenExtract content
     * Field
     * If content isRegularString，thenReturn directly
     * 
     * @param content originalinitialContent
    ...
    private String extractContentFromString(String content) {
        ...
        try {
            ...
            // Ifnotishaseffect JSON，Return directlyoriginalContent
        }

        // Ifnotis JSON Formatornohas content Field，Return directlyoriginalContent
        return content;
    }
    ...
    public boolean isAudioOwnedByAgent(String audioId, String agentId) {
        // QueryWhetherhasSpecifyAudioidandAgentidData，If there is one and only one record, it indicates this data belongs to this agent
