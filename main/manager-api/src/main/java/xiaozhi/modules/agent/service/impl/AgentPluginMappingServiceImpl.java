...
/**
 * @description forfortable【ai_agent_plugin_mapping(AgentandPluginUniqueMappingtable)】DatalibOperationServiceImplement
    ...
    public List<AgentPluginMapping> agentPluginParamsByAgentId(String agentId) {
        ...
        if (knowledgeBaseMap.size() > 0) {
            for (String pluginCode : knowledgeBaseMap.keySet()) {
                ...
                String description = "IfUserInquiry and【"
                        + String.join(",", knowledgeBaseList.stream().map(KnowledgeBaseEntity::getName).toList())
                        + "】This method should be called when covering content related to subject scope，Used forQuery：" + String.join(",",
