...
public class ServerSecretFilter extends AuthenticatingFilter {
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        // Gettokenand validate
        String token = getRequestToken((HttpServletRequest) servletRequest);
        if (StringUtils.isBlank(token)) {
            // tokenis empty，Return401
            this.sendUnauthorizedResponse((HttpServletResponse) servletResponse, "Server secret keycannot be empty");
            return false;
        }

        // VerifytokenWhether matched
        String serverSecret = getServerSecret();
        if (StringUtils.isBlank(serverSecret) || !serverSecret.equals(token)) {
            // tokenInvalid，Return401
            this.sendUnauthorizedResponse((HttpServletResponse) servletResponse, "InvalidServer secret key");
            return false;
        }
    ...
    private void sendUnauthorizedResponse(HttpServletResponse response, String message) {
        ...
        try {
            ...
            log.error("ResponseOutputFail", e);
        }
    }
