...
/**
 * RAGFlowKnowledge base adapterImplementation
 * <p>
 * reStructureDescription (Refactoring Note):
 * thisClassAlreadyUpgradeForUse {@link RAGFlowClient} UnifiedProcess HTTP Communication。
 * resolveResolveOldCodeMiddle Timeout Missing、Error Handling DisperseQuestion。
 * </p>
 */
@Slf4j
public class RAGFlowAdapter extends KnowledgeBaseAdapter {
    ...
    // Client Instance，InitializewhenCreate
    ...
    public void initialize(Map<String, Object> config) {
        ...
        // Initialize Client，DefaultExceedwhen 30s，canPass config Extension
        int timeout = 30;
        ...
        if (timeoutObj != null) {
            try {
                ...
                log.warn("ParsingExceedwhenConfigFailed，UseDefault value 30s");
            }
        }
        this.client = new RAGFlowClient(baseUrl, apiKey, timeout);
        log.info("RAGFlowAdapterInitializeComplete，ClientAlreadyReady");
    }
    ...
    public boolean validateConfig(Map<String, Object> config) {
        ...
        if (apiKey.contains("your_") || apiKey.contains("\u4f60")) {
    ...
    /**
     * Helper method: supports multiple key names to obtain configuration (compatible with camelCase and snake_case)
     */
    ...
    private RAGFlowClient getClient() {
        if (this.client == null) {
            // AttemptreNewInitialize
            if (this.config != null) {
                initialize(this.config);
            } else {
                throw new RenException(ErrorCode.RAG_CONFIG_NOT_FOUND, "Adapter uninitialized"); // Should throw RuntimeException
            }
        }
        return this.client;
    }
    ...
    public PageData<KnowledgeFilesDTO> getDocumentList(String datasetId, DocumentDTO.ListReq req) {
        try {
            log.info("=== [RAGFlow] GetDocument list: datasetId={} ===", datasetId);

            // Use Jackson Will DTO TransferFor Map doForQueryParameter
            ...
            log.error("GetDocument listFailed", e);
    ...
    public DocumentDTO.InfoVO getDocumentById(String datasetId, String documentId) {
        try {
            log.info("=== [RAGFlow] GetDocument details: datasetId={}, documentId={} ===", datasetId, documentId);
            ...
            log.error("GetDocument detailsFailed: documentId={}", documentId, e);
            throw convertToRenException(e);
        }
    }
    ...
    public KnowledgeFilesDTO uploadDocument(DocumentDTO.UploadReq req) {
        ...
        try {
            ...
            if (req.getChunkMethod() != null) {
                // WillEnumValueTransferFor RAGFlow PeriodpendingString（Like NAIVE -> naive）
            ...
            log.error("DocumentUpload failed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public void deleteDocument(String datasetId, DocumentDTO.BatchIdReq req) {
        try {
            ...
            log.error("Batch delete documentsFailed: datasetId={}", datasetId, e);
            throw convertToRenException(e);
        }
    }
    ...
    public boolean parseDocuments(String datasetId, List<String> documentIds) {
        try {
            log.info("=== [RAGFlow] ParsingDocument ===");
            ...
            log.error("ParsingDocumentFailed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public ChunkDTO.ListVO listChunks(String datasetId, String documentId, ChunkDTO.ListReq req) {
        try {
            // [promptLanternreStructure] Use objectMapper DynamicConvertQueryParameter，EliminateexceptHardCode
            Map<String, Object> params = objectMapper.convertValue(req, new TypeReference<Map<String, Object>>() {
            });
            ...
            log.error("GetChunkFailed: docId={}", documentId, e);
            throw convertToRenException(e);
        }
    }
    ...
    public RetrievalDTO.ResultVO retrievalTest(RetrievalDTO.TestReq req) {
        try {
            // [Production Reinforce] Defensive parameter alignment: RAGFlow Python side is sensitive to 0 or negative pagination
            // resolveResolve ValueError('Search does not support negative slicing.')
            ...
            // [Lantern Refactoring] Directly transparently pass strongly typed DTO, serialization handled by getClient
    ...
    public DatasetDTO.InfoVO createDataset(DatasetDTO.CreateReq req) {
        try {
            // [Production Fix] ReinforceDefault valueProcess，Prevent RAGFlow API error due to empty string or missing field (Code 101)
            // resolveResolve "Field: <avatar> - Message: <Missing MIME prefix>" EtcValidationFailed
            ...
            // 🤖 AutomaticCompleteEmbedding model：optimizefirstUseRequestPass parameters，secondly use default model in configuration
            if (StringUtils.isBlank(req.getEmbeddingModel())) {
                ...
                if (StringUtils.isNotBlank(defaultModel)) {
                    log.info("RAGFlow: Use default embedding model in configuration: {}", defaultModel);
                    req.setEmbeddingModel(defaultModel);
                }
                // If there is no default value in config, leave blank for RAGFlow server default fallback (or throw business exception)
            }

            // 🖼️ Auto-complete avatar: if null, provide a 1x1 transparent pixel to prevent RAGFlow MIME prefix validation failure
            if (StringUtils.isBlank(req.getAvatar())) {
                req.setAvatar(
                        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==");
            }

            // Pass strongly typed request object directly to Client，Jackson willProcess JsonProperty Map
            Map<String, Object> response = getClient().post("/api/v1/datasets", req);

            // Safely retrieve data and fully map via DatasetDTO.InfoVO
            Object dataObj = response.get("data");
            if (dataObj != null) {
            ...
            log.error("CreateDatasetFailed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public DatasetDTO.InfoVO updateDataset(String datasetId, DatasetDTO.UpdateReq req) {
        try {
            // RAGFlow API UpdateRecommendationPathwith ID
            ...
            log.error("UpdateDatasetFailed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public DatasetDTO.BatchOperationVO deleteDataset(DatasetDTO.BatchIdReq req) {
        try {
            // RAGFlow Batch deleteInterfaceUse DELETE /api/v1/datasets
            ...
            log.error("Batch deleteDatasetFailed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public DatasetDTO.InfoVO getDatasetInfo(String datasetId) {
        try {
            ...
            // RAGFlow sideDoes not existThisDataset
            return null;
        } catch (Exception e) {
            log.warn("GetDatasetInfoFailed: datasetId={}, error={}", datasetId, e.getMessage());
            throw convertToRenException(e);
        }
    }
    ...
    public void postStream(String endpoint, Object body, Consumer<String> onData) {
        try {
            ...
            log.error("StreamingRequest failed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public Object postSearchBotAsk(Map<String, Object> config, Object body,
        ...
        // SearchBot is actually a wrapper for Dataset retrieval, or an unreleased API?
        // Assume RAGFlow NotHaveExplicit /searchbots Interfaceprovide SDK Invoke，AndYes Dataset Retrieval Or Chat。
        // ButAccording to BotDTO，ItYes /api/v1/searchbots/ask (Assume)
        // Here config cancanYesCoveruse，OrmepluralOnlyYesuse adapter InstanceAlreadyHave client。
        // But Bot cancanUseNotsame API Key？Usually Adapter InstanceBindOne Key。
        // If Bot UseSystem Key，ThenDirectlyuse getClient()。

        // TemporarywhenAssume endpoint /api/v1/searchbots/ask Exist（OrClassSimilar）
        // If it isStreaming:
        try {
    ...
    public void postAgentBotCompletion(Map<String, Object> config, String agentId, Object body,
            Consumer<String> onData) {
        // AgentBot Toshould /api/v1/agentbots/{id}/completions
        try {
    ...
    // Reuse original helper parsing method to maintain compatibility
    // [Bug Fix] No longer swallow deserialization exceptions, avoiding upper layer misjudgment of 'document deleted'
    private PageData<KnowledgeFilesDTO> parseDocumentListResponse(Object dataObj, long curPage, long pageSize) {
        ...
        if (documents == null || documents.isEmpty()) {
            // RAGFlow ExplicitReturnEmptyDocument list，ThisYesCombinemethod"TrueEmpty"
            return new PageData<>(new ArrayList<>(), 0);
        }
        ...
        for (Object docObj : documents) {
            try {
                // Single document conversion fault tolerance: deserialization failure of one document does not affect other documents
                ...
                log.warn("[RAGFlow] SingleDocument DTO ConvertFailed，SkipThisDocument: {}", e.getMessage());
            }
        }
    ...
    private KnowledgeFilesDTO parseUploadResponse(Object dataObj, String datasetId, MultipartFile file) {
        ...
        // Attempt to extract document from response dataID (documentId)
        if (dataObj != null) {
            try {
        ...
        if (result == null) {
            log.error("Not yetcanFromRAGFlowResponseMiddleExtractTodocumentId，ResponseContent: {}", dataObj);
            // A minimal object containing basic information should be returned here DTO AndNotYes null，PreventUpSwim NPE
    ...
    /**
     * Will RAGFlow StrongType InfoVO MapToInnerPartUse KnowledgeFilesDTO
     * EnsureAllAvailableField（Name、Size、Status、ConfigEtc）allObtainToFullSync
     */
