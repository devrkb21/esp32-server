...
/**
 * RAGFlow HTTP Client
 * UnifiedProcessHTTPCommunication、Authentication、ExceedwhenWithErrorParsing
 */
@Slf4j
public class RAGFlowClient {
    ...
    public RAGFlowClient(String baseUrl, String apiKey, int timeoutSeconds) {
        ...
        // [Reinforce] Compatible RAGFlow Return RFC 1123 DateFormat (Like: Tue, 10 Feb 2026 10:27:35 GMT)
        this.objectMapper
        ...
        // optimizeFirst from Spring UpDowndocget fromPooling RestTemplate Bean (Issue 3: ConnectionPooling)
        RestTemplate pooledTemplate = null;
        try {
            pooledTemplate = xiaozhi.common.utils.SpringContextUtils.getBean(RestTemplate.class);
        } catch (Exception e) {
            log.warn("Unable toFrom SpringContext GetPooling RestTemplate，WillDegradeForSimpleConnectionMode: {}", e.getMessage());
        }

        if (false) { // Force new RestTemplate for debugging
            this.restTemplate = pooledTemplate;
            log.debug("RAGFlowClient AlreadySucceededMountGlobalPooling RestTemplate");
        } else {
            // FallbackpartyCase：ConfigExceedwhenAndCreateSimple RestTemplate
    ...
    public Map<String, Object> postMultipart(String endpoint, MultiValueMap<String, Object> parts) {
        ...
        // To prevent garbled filenames, some environments may need Charset set, but in Multipart it is usually controlled by Part header
    ...
    private Map<String, Object> doExecute(String url, HttpMethod method, HttpEntity<?> requestEntity) {
        try {
            ...
            // Return data Field，If data Does not existThenReturnEntire map (Depending onAgentSituation，Usually RAGFlow Return code=0, data=...)
            // Compatibility handling: if external caller needs to check code, it has already been checked here.
            // UnifiedReturn wrap  code  map StillYesOnlyReturn data?
            // According to analysis report, old logic retrieved data after checking code==0.
            // Here we return entire Map, letting Adapter decide how to extract, or strip directly here?
            // Recommendation: for flexibility, return full Map, but throw error in Client layer when code!=0.
            return map;
     ...
     * Use Java 21 HttpClient Implementation
     *
     * @param endpoint APIsidePoint
     * @param body     RequestAgent
     * @param onData   DataCallback（Called once per row of data received）
     */
