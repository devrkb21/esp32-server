...
class SuppressInvalidHandshakeFilter(logging.Filter):
    """Filter out invalid handshake error logs (such as HTTPS access to WS port)"""
...
def _setup_websockets_logger():
    """Configure all websockets-related loggers to filter invalid handshake errors"""
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            # Attempt to get device-id from URL query parameters
            ...
            if "device-id" not in query_params:
                await websocket.send("Port normal, to test connection please start digital-human test")
                await websocket.close()
                return
            else:
        ...
        """Handle new connection, creating a separate ConnectionHandler each time"""
        ...
        try:
            ...
            try:
                ...
                if hasattr(websocket, "closed") and not websocket.closed:
                    ...
                    # If no closed attribute, attempt closing directly
                    await websocket.close()
            except Exception as close_error:
                self.logger.bind(tag=TAG).error(
                    f"Server-side error while force closing connection: {close_error}"
                )
    ...
    async def _http_response(self, websocket, request_headers):
        ...
        if request_headers.headers.get("connection", "").lower() == "upgrade":
            ...
            # If plain HTTP request, return "Server is running"
        ...
        try:
            async with self.config_lock:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Checking whether VAD and ASR types need update: {update_vad} {update_asr}"
                )
                ...
                self.logger.bind(tag=TAG).info("Configuration update task completed")
                return True
        except Exception as e:
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If device is in whitelist, skip token validation and allow connection
                return
            else:
