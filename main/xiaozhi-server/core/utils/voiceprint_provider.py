...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        if not self.original_url:
            ...
            if not self.api_key:
                ...
                # Construct identify endpoint URL
                ...
                # Check if valid speakers are configured
                if not self.speaker_ids:
                    logger.bind(tag=TAG).warning("No valid speakers configured, voiceprint recognition will be disabled")
                    self.enabled = False
                else:
                    # Run health check to verify server availability
                    if self._check_server_health():
                        self.enabled = True
                        logger.bind(tag=TAG).info(f"Voiceprint recognition enabled: API={self.api_url}, speakers={len(self.speaker_ids)}, similarity_threshold={self.similarity_threshold}")
                    else:
                        self.enabled = False
    ...
    def _check_server_health(self) -> bool:
        ...
        logger.bind(tag=TAG).info("Performing voiceprint server health check")
        ...
        try:
            ...
            logger.bind(tag=TAG).warning("Voiceprint recognition server connection timed out")
            is_healthy = False
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.bind(tag=TAG).info(f"Voiceprint identification elapsed time: {total_elapsed_time:.3f}s")
            ...
            logger.bind(tag=TAG).error(f"Voiceprint identification timed out: {elapsed:.3f}s")
            return None
        except Exception as e:
            elapsed = time.monotonic() - api_start_time
