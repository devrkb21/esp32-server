...
class OpusEncoderUtils:
    ...
    def __init__(self, sample_rate: int, channels: int, frame_size_ms: int):
        ...
        self.complexity = 10  # Maximum quality
    ...
    def encode_pcm_to_opus_stream(self, pcm_data: bytes, end_of_stream: bool, callback: Callable[[Any], Any]):
        """
        Encode PCM data to Opus format in streaming fashion

        Args:
            pcm_data: PCM byte data
            end_of_stream: Whether end of stream is reached
            callback: Opus processing callback
        ...
        # Handle remaining data on end of stream
        if end_of_stream and len(self.buffer) > 0:
            # Create last frame padded with zeros
    ...
    def _encode(self, frame: np.ndarray) -> Optional[bytes]:
        """Encode single frame of audio data"""
        try:
            # Skip encoding if encoder already released
