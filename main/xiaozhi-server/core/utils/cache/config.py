...
class CacheType(Enum):
    ...
    VOICEPRINT_HEALTH = "voiceprint_health"  # Voiceprint health check
...
class CacheConfig:
    ...
    def for_type(cls, cache_type: CacheType) -> "CacheConfig":
        """Return preset configuration according to cache type"""
        configs = {
            CacheType.LOCATION: cls(
                strategy=CacheStrategy.TTL, ttl=None, max_size=1000  # Manual eviction
            ),
            ...
            CacheType.CONFIG: cls(
                strategy=CacheStrategy.FIXED_SIZE, ttl=None, max_size=20  # Manual eviction
            ),
