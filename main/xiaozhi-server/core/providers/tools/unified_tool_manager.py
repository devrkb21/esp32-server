...
class ToolManager:
    """Unified tool manager that manages all types of tools"""
    ...
    def register_executor(self, tool_type: ToolType, executor: ToolExecutor):
        ...
        self.logger.debug(f"Registering tool executor: {tool_type.value}")
    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting {tool_type.value} tools: {e}")

        self._cached_tools = all_tools
        return all_tools

    def get_function_descriptions(self) -> List[Dict[str, Any]]:
        """Get function descriptions of all tools (OpenAI format)"""
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting statistics for {tool_type.value} tools: {e}")
