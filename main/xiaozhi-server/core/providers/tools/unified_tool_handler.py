...
class UnifiedToolHandler:
    ...
    async def _initialize(self):
        ...
        try:
            ...
            self.logger.debug("Unified tool handler initialization completed")

            # Output list of all currently supported tools
    ...
    async def _initialize_mcp_endpoint(self):
        ...
        try:
            ...
            # Retrieve MCP endpoint URL from config
            mcp_endpoint_url = self.config.get("mcp_endpoint", "")

            if (
                mcp_endpoint_url
                and "your" not in mcp_endpoint_url.lower()
    ...
    def _initialize_home_assistant(self):
        """Initialize Home Assistant prompts"""
        try:
    ...
    def upload_functions_desc(self):
        """Refresh function descriptions list"""
        self.tool_manager.refresh_tools()
        self.logger.info("Function descriptions list refreshed")
        ...
        try:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    self.logger.error(f"Cannot parse function parameters: {arguments}")
                    return ActionResponse(
                        action=Action.ERROR,
                        response="Cannot parse function parameters",
                    )

            self.logger.debug(f"Calling function: {function_name}, args: {arguments}")

            # Send tool call display message to device
            try:
    ...
    def _combine_responses(self, responses: List[ActionResponse]) -> ActionResponse:
        ...
        # If any error occurred, return the first error
