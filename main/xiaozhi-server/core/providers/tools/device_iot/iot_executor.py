...
class DeviceIoTExecutor(ToolExecutor):
        ...
        try:
            ...
            if tool_name.startswith("get_"):
                ...
                if len(parts) >= 3:
                    ...
                    if value is not None:
                        ...
                        response_failure = arguments.get(
                            "response_failure", f"Unable to get state of {device_name}"
                        )
                        return ActionResponse(
                ...
                if len(parts) >= 2:
                    ...
                    # Extract control parameters (excluding response parameters)
                    control_params = {
                        k: v
    ...
    async def _send_iot_command(
        ...
        raise Exception(f"Method {method_name} of device {device_name} not found")
        ...
        for descriptor in descriptors:
                        ...
                        "function": {
                            ...
                            "parameters": {
                                ...
                                "properties": {
                                    "response_success": {
                                        "type": "string",
                                        "description": "Friendly response on query success, must use {value} as placeholder",
                                    },
                                    "response_failure": {
                                        "type": "string",
                                        "description": "Friendly response on query failure",
                                    },
                                },
            ...
            if "methods" in descriptor:
                for method_name, method_info in descriptor["methods"].items():
                    ...
                    # Add method's original parameters
                    ...
                    parameters.update(
                        {
                            "response_success": {
                                "type": "string",
                                "description": "Friendly response on successful operation",
                            },
                            "response_failure": {
                                "type": "string",
                                "description": "Friendly response on failed operation",
                            },
                        }
                    )
    ...
    def has_tool(self, tool_name: str) -> bool:
        """Check if specified device-side IoT tool exists"""
