...
class ServerMCPClient:
    """Server-side MCP client for connecting to and managing MCP services"""
    ...
    def is_connected(self) -> bool:
        ...
        Returns:
            bool: True if client is connected and working, False otherwise
        """
        ...
        async with AsyncExitStack() as stack:
            try:
                ...
                # Establish SSEClient
                    ...
                    # Backward compatibility for deprecated config
                    if "API_ACCESS_TOKEN" in self.config:
                        headers["Authorization"] = f"Bearer {self.config['API_ACCESS_TOKEN']}"
                        self.logger.bind(tag=TAG).warning("You are using deprecated API_ACCESS_TOKEN. Please specify it in headers in .mcp_server_settings.json, e.g. 'Authorization': 'Bearer API_ACCESS_TOKEN'")
                   
                    # Select client based on transport type, default SSE
