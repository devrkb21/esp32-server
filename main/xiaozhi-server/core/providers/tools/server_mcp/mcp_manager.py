...
class ServerMCPManager:
    """Centralized manager for multiple server-side MCP services"""
    ...
    async def initialize_servers(self) -> None:
        ...
        # Output current supported server MCP tools list
        if hasattr(self.conn, "func_handler") and self.conn.func_handler:
            # Refresh tool cache to ensure server MCP tools are loaded
    ...
    def get_all_tools(self) -> List[Dict[str, Any]]:
        """Get function definitions of all tools"""
        return self.tools
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute tool call with retry and reconnection on failure"""
        logger.bind(tag=TAG).info(f"Executing server MCP tool {tool_name}, args: {arguments}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Raise exception on final attempt
