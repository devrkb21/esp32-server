...
async def connect_mcp_endpoint(mcp_endpoint_url: str, conn=None) -> MCPEndpointClient:
    ...
    if not mcp_endpoint_url or "your" in mcp_endpoint_url.lower() or mcp_endpoint_url == "null":
        return None
...
async def handle_mcp_endpoint_message(mcp_client: MCPEndpointClient, message: str):
    ...
    try:
        ...
        if not isinstance(payload, dict):
            logger.bind(tag=TAG).error("MCP endpoint message malformed")
            return

        # Handle result
        if "result" in payload:
            result = payload["result"]
            # Safely get message ID, fallback to 0 if None
            ...
            if msg_id == 1:  # mcpInitializeID
                ...
                if result is not None and isinstance(result, dict):
                    ...
                    logger.bind(tag=TAG).warning(
                        "MCP endpoint initialization response empty or malformed"
                    )
                return

            elif msg_id == 2:  # mcpToolsListID
                logger.bind(tag=TAG).debug("Received MCP endpoint tools list response")
                if (
                    result is not None
                    ...
                    logger.bind(tag=TAG).info(
                        f"Tools supported by MCP endpoint: {len(tools_data)}"
                    )
                    ...
                    if next_cursor:
                        ...
                        # Refresh tool cache to ensure MCP endpoint tools are included
                        if (
                        ...
                        logger.bind(tag=TAG).info(
                            f"MCP endpoint tools retrieval finished, total {len(mcp_client.tools)} tools"
                        )
                else:
                    logger.bind(tag=TAG).warning(
                        "MCP endpoint tools list response empty or malformed"
                    )
                return
        ...
        logger.bind(tag=TAG).error(f"Error detail: {traceback.format_exc()}")
...
async def send_mcp_endpoint_tools_list_continue(
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).info(f"Sending MCP endpoint tools list request with cursor: {cursor}")
    await mcp_client.send_message(message)
...
async def call_mcp_endpoint_tool(
    ...
    """
    Call specified MCP endpoint tool and wait for response
    """
        ...
        if isinstance(args, str):
            ...
            if not args.strip():
                ...
                try:
                    ...
                    try:
                        # Match all JSON objects with regex
                        ...
                        logger.bind(tag=TAG).error(
                            f"Parameter JSON parsing failed: {str(e)}, raw args: {args}"
                        )
                        raise ValueError(f"Parameter JSON parsing failed: {str(e)}")
        elif isinstance(args, dict):
            arguments = args
        else:
            raise ValueError(f"Invalid parameter type, expected string or dict, got: {type(args)}")

        # Ensure parameters are dict
        if not isinstance(arguments, dict):
            raise ValueError(f"Parameters must be a dict, got: {type(arguments)}")
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).info(
        f"Sending MCP endpoint tool call request: {actual_name}, args: {json.dumps(arguments, ensure_ascii=False)}"
    )
    await mcp_client.send_message(message)
    ...
    try:
        ...
        # If result is not expected format, convert to string
