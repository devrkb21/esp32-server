...
async def handle_mcp_message(
    ...
    """Handle MCP messages, including init, tool list, and tool call responses"""
    logger.bind(tag=TAG).debug(f"Handling MCP message: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message missing payload field or malformed")
        return
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                logger.bind(tag=TAG).info(
                    f"Tools supported by client device: {len(tools_data)}"
                )
...
async def call_mcp_tool(
    ...
    logger.bind(tag=TAG).info(f"Sending client MCP tool call request: {actual_name}, args: {args}")
    await send_mcp_message(conn, payload)
