...
class TTSProvider(TTSProviderBase):
    ...
    async def open_audio_channels(self, conn):
        try:
            ...
            # Update sample rate in audio_params to actual conn.sample_rate
    ...
    async def _ensure_connection(self):
        ...
        try:
            ...
            # Start monitoring task after connection established successfully
    ...
    async def finish_connection(self):
        """Send FinishConnection event, waiting for server to return EVENT_ConnectionFinished"""
        try:
            if self.ws:
    ...
    async def text_to_speak(self, text, _):
        ...
        try:
            ...
            if self.ws is None:
                logger.bind(tag=TAG).warning(f"WebSocket connection does not exist, terminating sending text")
                return
            ...
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    if res.optional.sessionId and self.conn.sentence_id != res.optional.sessionId:
                        # If session-end event, reset state even if session ID does not match
                    ...
                    if res.optional.event == EVENT_SessionCanceled:
                        ...
                        # Restore replaced text returned by TTS service to original text for subtitle display
                        ...
                        # In non-reuse mode, send FinishConnection after session finishes
    ...
    # Read string content from res array slice
    ...
    def get_payload_bytes(
        ...
        # Add to req_params if mix_speaker configuration exists
        if self.mix_speaker:
