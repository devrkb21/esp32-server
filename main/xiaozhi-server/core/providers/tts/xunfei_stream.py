                ...
                if ContentType.TEXT == message.content_type:
                    if message.content_detail:
                        try:
                            ...
                            # Do not continue, ensure subsequent processing is not interrupted
    ...
    async def close(self):
        ...
        if self._monitor_task:
            try:
                ...
                logger.bind(tag=TAG).warning(f"Error cancelling monitoring task on close: {e}")
            self._monitor_task = None
        ...
        try:
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    if self.conn.client_abort:
                        logger.bind(tag=TAG).info("Received abort signal, terminating TTS response monitoring")
                        break
                        ...
                        if code == 0:
                            ...
                            if audio_payload:
                                ...
                                if status == 0:
                                    ...
                                    logger.bind(tag=TAG).debug("Received end state audio data, TTS synthesis completed")
                                    self.activate_session = False
