...
class AccessToken:
    ...
    def create_token(access_key_id, access_key_secret, timeout):
        parameters = {
            ...
            "Action": "CreateToken",
            "Format": "JSON",
            "RegionId": "cn-shanghai",  # Use cn-shanghai region for Token retrieval
...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        # Voice configuration - CosyVoice LLM voice
        ...
        # If configured with internal address (containing -internal.aliyuncs.com), use ws protocol, default is wss protocol
    ...
    async def _ensure_connection(self):
        ...
        try:
            if self._is_token_expired():
                logger.bind(tag=TAG).warning("Token has expired, automatically refreshing...")
                self._refresh_token()
            current_time = time.time()
            if self.ws and current_time - self.last_active_time < 10:
                # Only connections within 10 seconds can be reused for continuous conversation
                self.task_id = uuid.uuid4().hex
