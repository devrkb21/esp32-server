...
class TTSProvider(TTSProviderBase):
    ...
    async def text_streaming(self, text, output_file):
        try:
            # Asynchronously connect to WebSocket server using websockets
            async with websockets.connect(self.url) as ws:
                ...
                # Receive end response to avoid server exception
                await ws.recv()

                # Decide whether to save file according to config
