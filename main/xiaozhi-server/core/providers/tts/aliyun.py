...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        if self.access_key_id and self.access_key_secret:
            # Generate temporary token using secret pair
            self._refresh_token()
        else:
    ...
    def _refresh_token(self):
        """Refresh Token and record expiry time"""
        if self.access_key_id and self.access_key_secret:
            self.token, expire_time_str = AccessToken.create_token(
                self.access_key_id, self.access_key_secret, self.tts_timeout
            )
            if not expire_time_str:
                raise ValueError("Unable to obtain valid Token expiry time")

            try:
                # Convert uniformly to string for processing
    ...
    async def text_to_speak(self, text, output_file):
        ...
        try:
            ...
            # Check whether mime type of returned request data starts with audio/***, if so save to designated path; returns binary format
