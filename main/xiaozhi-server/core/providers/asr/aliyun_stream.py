...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # If intranet address is configured (contains -internal.aliyuncs.com), use ws protocol, default is wss protocol
    ...
    async def receive_audio(self, conn, pcm_frame, audio_have_voice):
        # Call parent class method first to handle basic logic
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        logger.bind(tag=TAG).debug("Start request sent, waiting for server to become ready...")
        ...
        try:
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    if status != 20000000:
                        if status == 40010004:
                            logger.bind(tag=TAG).warning(f"Please close connection after server finishes responding, status code: {status}")
                            break
                    ...
                    # Receiving TranscriptionStarted indicates server is ready to receive audio data
                    if message_name == "TranscriptionStarted":
                        ...
                        # Sentence end (triggers on every sentence)
                        ...
                        if text:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # In manual mode, only trigger processing after receiving stop signal (process once only)
