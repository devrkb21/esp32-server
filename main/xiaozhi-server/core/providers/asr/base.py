...
class ASRProviderBase(ABC):
    ...
    async def receive_audio(self, conn: "ConnectionHandler", pcm_frame, audio_have_voice):
        if conn.client_listen_mode == "manual":
            ...
            # Auto/realtime mode: use VAD detection
            conn.asr_audio.append(pcm_frame)

            # If no voice and previously no voice, cache partial audio
            ...
            if conn.asr.interface_type != InterfaceType.STREAM and conn.client_voice_stop:
                # Directly use PCM data from asr_audio
                pcm_bytes = b"".join(conn.asr_audio)
                # Check if enough audio data is present (1920 bytes/frame, 15 frames ~28800 bytes)
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            if conn.voiceprint_provider and wav_data:
                ...
                # Concurrently await both results
            ...
            # Log recognition result - check if exception
    ...
    def prefers_temp_file(self) -> bool:
        """Whether temporary file is preferred"""
        return False
    ...
    async def speech_to_text(
        ...
        :param artifacts: Audio artifacts containing PCM data, file paths, etc.
        :return: Recognition result text and file path (if any)
        """
        pass
