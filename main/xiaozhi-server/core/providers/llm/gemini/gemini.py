...
def setup_proxy_env(http_proxy: str | None, https_proxy: str | None):
    """
    Test whether HTTP and HTTPS proxies are reachable, and set environment variables.
    If HTTPS proxy is unreachable but HTTP works, point HTTPS_PROXY to HTTP as well.
    """
    ...
    if http_proxy:
        ...
        if ok_http:
            ...
            log.bind(tag=TAG).info(f"Gemini HTTP proxy connection successful: {http_proxy}")
        else:
    ...
    if https_proxy:
        ...
        if ok_https:
            ...
            log.bind(tag=TAG).info(f"Gemini HTTPS proxy connection successful: {https_proxy}")
        else:
            log.bind(tag=TAG).warning(
                f"Gemini HTTPS proxy unavailable: {https_proxy}"
            )

    # If https_proxy is unavailable, but http_proxy can reach https, reuse http_proxy as https_proxy
    ...
    if not ok_http and not ok_https:
        log.bind(tag=TAG).error(
            f"Gemini proxy setup failed: Both HTTP and HTTPS proxies are unavailable, please check config"
        )
        raise RuntimeError("Both HTTP and HTTPS proxies are unavailable, please check config")
...
class LLMProvider(LLMProviderBase):
    def __init__(self, cfg: Dict[str, Any]):
        ...
        if http_proxy or https_proxy:
            log.bind(tag=TAG).info(
                f"Detected Gemini proxy configuration, testing connectivity and configuring environment..."
            )
    ...
    # Gemini docs indicate no session_id is needed; construct directly from dialogue
        ...
        try:
            for chunk in stream:
                ...
                for part in cand.content.parts:
                    # a) Function call - usually only the last utterance is a function call
    ...
    # Close stream, reserved for dialogue interruption feature
    @staticmethod
