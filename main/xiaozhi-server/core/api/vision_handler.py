...
class VisionHandler(BaseHandler):
    ...
    def _verify_auth_token(self, request) -> Tuple[bool, Optional[str]]:
        ...
        # Test mode: allow specific test token or skip verification
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if not is_valid_image_file(image_data):
                raise ValueError(
                    "Unsupported file format; please upload a valid image (JPEG, PNG, GIF, BMP, TIFF, WEBP supported)"
                )

            # Convert image to base64 encoding
            image_base64 = base64.b64encode(image_data).decode("utf-8")

            # If management console enabled, fetch model configuration from console
            ...
            if not select_vllm_module:
                raise ValueError("Default visual analysis module has not been configured")
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            if vision_explain and len(vision_explain) > 0 and "null" != vision_explain:
                message = (
                    f"MCP Vision endpoint is running normally. Visual analysis endpoint is: {vision_explain}"
                )
            else:
                message = "MCP Vision endpoint is not running properly. Please check data/.config.yaml and configure server.vision_explain"
