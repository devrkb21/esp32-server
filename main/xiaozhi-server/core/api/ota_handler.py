...
class OTAHandler(BaseHandler):
    ...
    def _get_websocket_url(self, local_ip: str, port: int) -> str:
        ...
        if "your" not in websocket_config.lower():
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If non-empty string configured
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # If signing fails, leave empty; device decides if passwordless is allowed
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signature key, password left empty")

                # Build MQTT configuration (directly using mqtt_gateway string)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Issuing MQTT gateway configuration for device {device_id}")

            else:  # mqtt_gateway not configured, issue WebSocket
                # If authentication enabled, perform verification
                token = ""
                if self.auth_enable:
            ...
            try:
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Issuing firmware {chosen_version} for device {device_id} [if URL prefix is incorrect, check server.vision_explain in config]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            message = f"OTA endpoint is running normally. WebSocket address sent to device is: {websocket_url}"
