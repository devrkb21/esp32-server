...
class ServerTextMessageHandler(TextMessageHandler):
    ...
    async def handle(self, conn, msg_json: Dict[str, Any]) -> None:
        # If config is read from API, verify secret
        if not conn.read_config_from_api:
            return
        # Get secret from post request
        post_secret = msg_json.get("content", {}).get("secret", "")
        secret = conn.config["manager-api"].get("secret", "")
        # If secret does not match, return error
                ...
                json.dumps(
                    {
                        "type": "server",
                        "status": "error",
                        "message": "Server secret key validation failed",
                    }
                )
            )
            return
                        ...
                        json.dumps(
                            {
                                "type": "server",
                                "status": "error",
                                "message": "Unable to obtain server instance",
