...
class AuthManager:
    ...
    Generates and verifies client_id, device_id, token (HMAC-SHA256) auth tuple.
    Token contains no plaintext client_id/device_id, carrying only signature + timestamp;
    client_id/device_id are passed upon connection.
    ...
    def generate_token(self, client_id: str, username: str) -> str:
        ...
        Args:
            ...
            username: Device username (typically deviceId)
        Returns:
            str: Token string
        """
        ts = int(time.time())
        ...
        # Token contains only signature and timestamp, without plaintext info
