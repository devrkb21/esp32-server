...
description = "Streaming TTS first-word latency testing"
class StreamTTSPerformanceTester:
    ...
    async def test_aliyun_tts(self, text=None, test_count=5):
        """Test Aliyun streaming TTS first-word latency"""
        ...
        for i in range(test_count):
            try:
                ...
                # Unified timing start: begin timing before establishing connection
                ...
                async with websockets.connect(ws_url, extra_headers={"X-NLS-Token": token}) as ws:
                    ...
                    while True:
                        ...
                        if isinstance(response, bytes):
                            ...
                            print(f"[AliyunTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                            break
                ...
                print(f"[AliyunTTS] Attempt {i+1} test failed: {str(e)}")
                latencies.append(None)
        
        return self._calculate_result("AliyunTTS", latencies, test_count)

    async def test_alibl_tts(self, text=None, test_count=5):
        """Test DashScope CosyVoice streaming TTS first-word latency"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # 2. Wait for task-started event (crucial! must wait before sending text)
                    task_started = False
                    while not task_started:
                        ...
                        if isinstance(msg, str):
                            ...
                            if event == "task-started":
                                task_started = True
                                print(f"[DashScopeTTS] Attempt {i+1} task started successfully")
                            elif event == "task-failed":
                                raise Exception(f"Failed to start: {header.get('error_message', 'Unknown error')}")

                    # 3. Send continue-task (send text)
                    continue_task_message = {
                        "header": {
                            "action": "continue-task",  # continue-task
                            "task_id": session_id,
                            "streaming": "duplex",
                        },
                    ...
                    # 5. Wait for first audio chunk
                    while True:
                        msg = await asyncio.wait_for(ws.recv(), timeout=15.0)
                        if isinstance(msg, (bytes, bytearray)) and len(msg) > 0:
                            latency = time.time() - start_time
                            print(f"[DashScopeTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                            latencies.append(latency)
                            break
                ...
                print(f"[DashScopeTTS] Attempt {i+1} failed: {str(e)}")
                latencies.append(None)

        return self._calculate_result("DashScopeTTS", latencies, test_count)

    async def test_doubao_tts(self, text=None, test_count=5):
        """Test Volcengine streaming TTS first-word latency"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(ws_url, additional_headers=ws_header, max_size=1000000000) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[VolcengineTTS] Attempt {i+1} First word latency: {latency:.3f}s")

            except Exception as e:
                print(f"[VolcengineTTS] Attempt {i+1} test failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_paddlespeech_tts(self, text=None, test_count=5):
        """Test PaddleSpeech streaming TTS first-word latency"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(tts_url) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[PaddleSpeechTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                ...
                print(f"[PaddleSpeechTTS] Attempt {i+1} test failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_indexstream_tts(self, text=None, test_count=5):
        """Test IndexStream streaming TTS first-word latency"""
        text = text or self.test_texts[0]
        latencies = []
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(api_url, json=payload, timeout=10) as resp:
                        ...
                        async for chunk in resp.content.iter_any():
                            ...
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[IndexStreamTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                            resp.close()
                            break
                        else:
                            latencies.append(None)
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(api_url, params=params, headers=headers, timeout=10) as resp:
                        ...
                        async for _ in resp.content.iter_any():
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[LinkeraiTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                            break
                        else:
                            latencies.append(None)

            except Exception as e:
                print(f"[LinkeraiTTS] Attempt {i+1} test failed: {str(e)}")
                latencies.append(None)
        
        return self._calculate_result("LinkeraiTTS", latencies, test_count)
    
    async def test_xunfei_tts(self, text=None, test_count=5):
        """Test Xunfei streaming TTS first-word latency"""
        text = text or self.test_texts[0]
        latencies = []
        
        for i in range(test_count):
            try:
                # Fix config node name to match XunFeiTTS in config file
                    ...
                    while not first_audio_received:
                        ...
                        if audio_payload:
                            ...
                            if status == 1 and audio_data:
                                # Received first audio chunk
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[XunfeiTTS] Attempt {i+1} First word latency: {latency:.3f}s")
                                first_audio_received = True
                                break
            except Exception as e:
                print(f"[XunfeiTTS] Attempt {i+1} test failed: {str(e)}")
                latencies.append(None)
        
        return self._calculate_result("XunfeiTTS", latencies, test_count)
    ...
    def _build_xunfei_request(self, app_id, text, voice):
        """Build Xunfei TTS request structure"""
        return {
            "header": {
                "app_id": app_id,
                "status": 2,
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (properly handle None, exclude failed tests)"""
        # Exclude failed tests (None and <=0 latency), only count valid latencies
    ...
    def _print_results(self, test_text, test_count):
        ...
        print(f"Test count: {test_count} times per TTS service")

        # Sort results: successful first, by latency ascending
        ...
        print("\nTest Notes: Measure time from establishing connection to receiving first audio chunk, average of multiple tests")
        print("- Timing start: Before establishing WebSocket/HTTP connection")
        print("- Timeout control: Max wait time for single request is 10s")
        print("- Error handling: Failed tests excluded from average; only successful test latencies counted")
        print("- Sort rule: Sorted by average duration ascending")
    ...
    async def run(self, test_text=None, test_count=5):
        ...
        Args:
            test_text: Text to test, or default text if None
            test_count: Number of tests per TTS service
        """
        test_text = test_text or self.test_texts[0]
        print(f"Starting streaming TTS first-word latency testing...")
        print(f"Test text: {test_text}")
        print(f"Test count per TTS service: {test_count}")
        
        if not self.config.get("TTS"):
            print("No TTS config found in configuration")
            return
...
async def main():
    ...
    parser.add_argument("--count", type=int, default=5, help="Test count per TTS service")
