...
description = "Vision model performance testing"
...
class AsyncVisionPerformanceTester:
    ...
    async def _test_vllm(self, vllm_name: str, config: Dict) -> Dict:
        """Asynchronously test single vision model performance"""
        try:
            ...
            # Concurrently execute all tests
            test_results = await asyncio.gather(*test_tasks)

            # Process results
            valid_results = [r for r in test_results if r is not None]
            if not valid_results:
                print(f"⚠️  {vllm_name} has no valid data, possible configuration error")
                return {"name": vllm_name, "type": "vllm", "errors": 1}
    ...
    def _print_results(self):
        ...
        if vllm_table:
            ...
            print("\n⚠️ No available vision models to test.")
    ...
    async def run(self):
        ...
        if not self.test_images:
            print(f"\n⚠️  No image files found in {self.image_root}, cannot perform test")
            return
