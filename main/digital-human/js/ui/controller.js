...
class UIController {
    ...
    initEventListeners() {
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    this.addChatMessage('⚠️ Camera failed to start, please check browser permissions', false);
                                }
                            }).catch(error => {
        ...
        // Click modal backdrop to close (disabled for specific modals)
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => {
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    // settingsModal, mcpToolModal, mcpPropertyModal can only be closed via close button
                    const nonClosableModals = ['settingsModal', 'mcpToolModal', 'mcpPropertyModal'];
                    if (nonClosableModals.includes(modal.id)) {
                        return; // Backdrop click disabled
                    }
                    this.hideModal(modal.id);
                }
            });
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Recording unavailable over HTTP; text chat only' : 'Microphone unavailable';
                } else {
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = 'Record';
            recordBtn.title = isHttpNonLocalhost ? 'Recording unavailable over HTTP; text chat only' : 'Microphone unavailable';
    ...
    switchLive2DModel() {
        const modelSelect = document.getElementById('live2dModelSelect');
        if (!modelSelect) {
            console.error('Model select dropdown does not exist');
            return;
        }
        ...
        if (app && app.live2dManager) {
            app.live2dManager.switchModel(selectedModel)
                ...
                .catch(error => {
                    ...
                    this.addChatMessage('Error switching model', false);
                });
        } else {
    ...
    async handleApplyWakeword() {
        ...
        if (payload.enabled && payload.wakeWords.length === 0) {
            this.addChatMessage('At least one wake word is required when wake words are enabled.', false);
            return;
        }
        ...
        try {
            ...
            // Compare new address with current connected address
            ...
            if (urlChanged) {
                // Address changed, confirm first
                const shouldRestart = window.confirm('Address changed. Continue? (Will disconnect and reconnect)');
                if (!shouldRestart) {
                    // Restore old address in localStorage
                    localStorage.setItem('xz_tester_wakewordWsUrl', currentWsUrl);
                    this.addChatMessage('Address change cancelled.', false);
                    return;
                }
                ...
                // Start new connection (automatically with new address)
                startWakewordBridgeListener();

                // Wait for bridge_connected
                await new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        reject(new Error('Connection to new server timed out'));
                    }, 5000);
                ...
                this.addChatMessage('Wake word config saved, wake word service is restarting.', false);
            } else {
                // Address unchanged: operate directly on current connection
                const response = await requestWakewordBridge('set_wakeword_config', payload);
                this.applyWakewordConfig(response.payload || payload);

                const shouldRestart = window.confirm('Wake words saved. Restart wake word service now to take effect?');
                if (!shouldRestart) {
                    this.addChatMessage('Wake word config saved; will take effect after restarting service.', false);
                    return;
                }
            ...
            this.addChatMessage(`Failed to apply wake words: ${error.message}`, false);
        } finally {
            if (applyWakewordBtn) {
                applyWakewordBtn.disabled = false;
                applyWakewordBtn.textContent = 'Apply Wake Words';
            }
        }
    }
    ...
    startAIChatSession() {
        this.addChatMessage('Connected successfully, start chatting! 😊', false);
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Due to HTTP access, microphone recording is unavailable; text chat only', false);
            } else {
                this.addChatMessage('⚠️ Microphone is unavailable, please check permissions; text chat only', false);
            }
        }
        ...
        if (window.cameraAvailable && typeof window.startCamera === 'function') {
            window.startCamera().then(success => {
                if (success) {
                    ...
                    this.addChatMessage('⚠️ Camera failed to start, possibly denied by browser', false);
                }
            }).catch(error => {
                log(`Camera start error: ${error.message}`, 'error');
            });
    ...
    async handleConnect() {
        ...
        if (this.isConnecting || (wsHandler && wsHandler.isConnected())) {
            log('Connection already active or in progress, ignoring dial request', 'info');
            return;
        }
    ...
    async triggerWakewordDial(wakeWord = 'Wake Word') {
        ...
        if (this.isConnecting || this.dialBtnDisabled) {
            log('Page is connecting, ignoring repeated wake event', 'info');
            return false;
        }

        if (now - this.lastWakewordDialTime < 3000) {
            log('Wake triggers too frequent, ignoring this auto dial', 'warning');
            return false;
        }

        this.lastWakewordDialTime = now;
        this.addChatMessage(`Detected wake word "${wakeWord}", preparing to connect to server...`, false);
