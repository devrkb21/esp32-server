export default class BlockingQueue {
    ...
    enqueue(item, ...restItems) {
        ...
        // If there are additional parameters, batch process all items
        else {
    ...
    async dequeue(min = 1, timeout = Infinity, onTimeout = null) {
        // 1. If empty, wait for first data arrival (all calls share same promise)
    ...
    /* Internal: after data changes, check which waiters are satisfied */
    #wakeWaiters() {
    ...
    /* Clear queue (keep object reference, does not affect waiters) */
    clear() {
        this.#items.length = 0;
    }
}