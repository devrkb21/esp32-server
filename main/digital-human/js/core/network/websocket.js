...
export class WebSocketHandler {
    ...
    _sendWakeupMessages(sessionId) {
        ...
        log('Sent listen detect message, wake word: Hey hello', 'info');
    ...
    handleTextMessage(message) {
        if (message.type === 'hello') {
            ...
            log('Connected successfully, camera is now available', 'success');
            ...
            if (message.text && (message.text.includes('\u7ed1\u5b9a') || message.text.toLowerCase().includes('bind'))) {
            ...
            // Display STT message via chat message callback
            if (this.onChatMessage && message.text) {
                this.onChatMessage(message.text, true);
            }
        } else if (message.type === 'llm') {
            log(`LLM response: ${message.text}`, 'info');
            // Display LLM response via chat message callback
            if (this.onChatMessage && message.text) {
                this.onChatMessage(message.text, false);
            }

            // If contains emojis, update sessionStatus emoji and trigger Live2D action
            ...
            // Only add to conversation when text is not just emojis
            // Check if content remains after stripping emojis
    ...
    handleTTSMessage(message) {
        if (message.state === 'start') {
            ...
            // Do not clear animation at sentence end, wait for next sentence or final stop
        } else if (message.state === 'stop') {
            log('Server speech transmission ended, clearing all audio buffers', 'info');
            ...
            // Delay stopping Live2D talking animation to ensure all sentences complete
            setTimeout(() => {
        ...
        try {
            ...
            if (live2dManager) {
                ...
                if (live2dManager.initializeAudioAnalyzer()) {
                    log('Live2D audio analyser initialized, connected to audio player', 'success');
                } else {
                    log('Live2D audio analyser initialization failed, will use simulated animation', 'warning');
                }
            }
        } catch (error) {
    ...
    handleMCPMessage(message) {
        ...
        log(`Server message: ${JSON.stringify(message)}`, 'info');
        ...
        if (payload.method === 'tools/list') {
            ...
            log(`Call tool: ${toolName} args: ${JSON.stringify(toolArgs)}`, 'info');
            ...
            // Save vision analysis endpoint URL
    ...
    async connect() {
        ...
        try {
            ...
            // Set binary receive type to ArrayBuffer
    ...
    setupEventHandlers() {
        this.websocket.onopen = async () => {
            ...
            // Default state is listening after connection succeeds
            this.isRemoteSpeaking = false;
            if (this.onSessionStateChange) {
                this.onSessionStateChange(false);
            }

            // Initialize Live2D audio analyser upon successful WebSocket connection
        ...
        this.websocket.onmessage = (event) => {
            try {
                ...
                // No longer using old addMessage function as conversationDiv element does not exist
    ...
    sendTextMessage(text) {
        ...
        try {
            // If peer is speaking, send abort message first
            if (this.isRemoteSpeaking && this.currentSessionId) {
                ...
                log('Sending abort message', 'info');
            }
