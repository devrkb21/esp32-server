...
export class AudioPlayer {
        ...
        try {
            if (typeof window.ModuleInstance === 'undefined') {
                if (typeof Module !== 'undefined') {
                    ...
                    throw new Error('Opus library not loaded, neither ModuleInstance nor Module object exists');
                }
            }
    ...
    async startAudioBuffering() {
        ...
        while (true) {
            const packets = await this.queue.dequeue(
                ...
                timeout,
                (count) => {
                    log(`Buffer timeout, current buffered packet count: ${count}, starting playback`, 'info');
                }
            );
            if (packets.length) {
    ...
    getAudioStats() {
        ...
        return {
            ...
            totalPending: pendingDecode + pendingPlay  // Total packets pending processing
        };
    }
    ...
    clearAllAudio() {
        ...
        // Clear receive queue (using clear method to preserve object reference)
        this.queue.clear();

        // Clear all stream context buffers
        if (this.streamingContext) {
