...
export async function initMcpTools() {
    ...
    if (savedTools) {
        try {
            ...
            // Merge default tools and saved tools, preserving user custom tools
            const defaultToolNames = new Set(defaultMcpTools.map(t => t.name));
            // Add new tools not in default tools
    ...
    container.innerHTML = mcpProperties.map((prop, index) => `
        <div class="mcp-property-card" onclick="window.mcpModule.editMcpProperty(${index})">
            <div class="mcp-property-row-label">
                <span class="mcp-property-label">Param Name</span>
...
/**
 * Add parameter - Open parameter modal
 */
...
/**
 * Edit parameter - Open parameter modal
 */
...
/**
 * Update number range input visibility
 */
...
export async function executeMcpTool(toolName, toolArgs) {
    ...
    // Handle camera photo tool
    if (toolName === 'self_camera_take_photo') {
        if (typeof window.takePhoto === 'function') {
            const question = toolArgs && toolArgs.question ? toolArgs.question : 'Describe what is seen in the image';
            log(`Taking photo: ${question}`, 'info');
            const result = await window.takePhoto(question);
            return result;
        } else {
            log('Photo capture feature unavailable', 'warning');
            return { success: false, error: 'Camera not started or photo capture unsupported' };
        }
    }

    // If mock response exists, use it
