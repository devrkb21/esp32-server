...
// Helper function: convert Base64 data to Blob
...
class App {
    ...
    async init() {
        ...
        initMcpTools();
        // Initialize local wake word event listener
    ...
    async initLive2D() {
        try {
            ...
            const live2dStatus = document.getElementById('live2dStatus');
            if (live2dStatus) {
                live2dStatus.textContent = '● Failed to load';
    ...
    /**
     * Check microphone availability
     * Called during app init to check mic availability and update UI
     */
    ...
    checkCameraAvailability() {
        ...
        log('Camera availability check complete: bound verification code by default', 'success');
    }
    ...
    async initCamera() {
        ...
        return new Promise((resolve) => {
            window.startCamera = async () => {
                try {
                    ...
                    if (error.name === 'NotAllowedError') {
                        ...
                        log('Camera is already in use by another program', 'warning');
                    }
                    return false;
                }
            };
            ...
            window.takePhoto = (question = 'Describe what is seen in the image') => {
                return new Promise(async (resolve) => {
                    ...
                    if (!video || video.readyState !== video.HAVE_ENOUGH_DATA) {
                        ...
                        resolve({
                            success: false,
                            error: 'Camera not ready, please ensure connected and camera is started'
                        });
                        return;
                    }
                    ...
                    log(`Photo captured successfully, image data length: ${photoData.length}`, 'success');
                    ...
                    try {
                        ...
                        if (xz_tester_vision) {
                            ...
                            try {
                                ...
                                throw new Error(`Failed to parse vision configuration`);
                            }

                            const { url, token } = visionInfo || {};
                            if (!url || !token) {
                                throw new Error('Vision analysis failed: config missing endpoint URL or token');
                            }
                        ...
                        resolve({
                            ...
                            vision_analysis: {
                                success: false,
                                error: error.message,
                                fallback: 'Unable to connect to vision analysis service'
                            }
                        });
                    }
                });
...
const app = new App();
// Expose app instance globally for access by other modules
window.chatApp = app;
