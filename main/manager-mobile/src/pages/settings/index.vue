...
async function testServerBaseUrl() {
  ...
  try {
    ...
    console.error('Failed to test server URL:', error)
    toast.error({
      msg: t('message.invalidAddress'),
      duration: 3000,
    })
...
async function saveServerBaseUrl() {
  ...
  // Handle config cache update issue
  uni.request({
    ...
    fail: (err) => {
      console.error('Failed to get SM2 public key:', err)
    },
  })

  // Clear all cache after switching request URL
...
// Restart app (native restart on App, navigate to home on others)
function restartApp() {
  // #ifdef APP-PLUS
...
onMounted(async () => {
  // Load server address settings in non-mini-program environment only
  if (!isMp) {
    loadServerBaseUrl()
  }
  getCacheInfo()
...
<template>
  <view class="min-h-screen bg-[#f5f7fb]">
    ...
    <view class="p-[24rpx]">
      <!-- Network settings - non-mini-program only -->
